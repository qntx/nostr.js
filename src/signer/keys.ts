...
import { decryptFromPubkey, encryptToPubkey } from "../nips/nip44.ts";
...
export class KeysSigner implements NostrSigner {
  ...
  async nip44Encrypt(peer: string, plaintext: string): Promise<string> {
    return encryptToPubkey(plaintext, this.#keys.secretKey.bytes, peer);
  }

  async nip44Decrypt(peer: string, payload: string): Promise<string> {
    return decryptFromPubkey(payload, this.#keys.secretKey.bytes, peer);
  }
}
