...
Version is `0.0.1` for a local complete publish (`npm publish` runs `build:wasm`). `pack:local` stays JS-only. Do not tag `v0.0.1`. After the npm trusted publisher is configured, `bun run release -- --minor` tags `v0.1.0` and Actions publishes with provenance.
