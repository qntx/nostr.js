...
Version is `0.0.0`. Local tarball (`bun run pack:local`) is JS-only — no `@qntx/nostr/wasm`. First registry publish: `bun run release -- --minor` (writes `v0.1.0`, pushes the tag); GitHub Actions `publish.yml` on `v*.*.*` runs `build:wasm` then `npm publish --provenance`. Configure npm trusted publisher for this repo/workflow before tagging. Do not publish `0.0.0` to the registry.
